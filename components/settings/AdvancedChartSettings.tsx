...
// ═══ Recent colors singleton (persists across picker instances) ═══
...
function addRecentColor(c: string) {
  const recent = getRecentColors().filter(r => r.toLowerCase() !== c.toLowerCase());
  recent.unshift(c);
  localStorage.setItem(RECENT_COLORS_KEY, JSON.stringify(recent.slice(0, 8)));
}

// ═══ HSV conversion helpers ═══
function _hsvToHex(h: number, s: number, v: number): string {
...
/** Pro HSV picker — SV square + hue bar + HEX/RGB inputs + copy + recent colors */
function InlineHSVPicker({ value, onChange }: { value: string; onChange: (color: string) => void }) {
  const [hue, setHue] = useState(0);
  const [sat, setSat] = useState(100);
  const [bright, setBright] = useState(100);
  const [hexInput, setHexInput] = useState(value);
  const [rgb, setRgb] = useState(_hexToRGB(value));
  ...
  useEffect(() => {
    ...
    const { h, s, v } = _hexToHSV(value);
    setHue(h); setSat(s); setBright(v);
    setHexInput(value);
    setRgb(_hexToRGB(value));
  }, [value]);
  ...
  useEffect(() => {
    ...
    gradH.addColorStop(1, _hsvToHex(hue, 100, 100));
  ...
  useEffect(() => {
    ...
    for (let i = 0; i <= 6; i++) grad.addColorStop(i / 6, _hsvToHex(i * 60, 100, 100));
  ...
  const emit = useCallback((h: number, s: number, v: number) => {
    ...
    setHexInput(hex);
    setRgb(_hexToRGB(hex));
    onChange(hex);
  }, [onChange]);

  const emitHex = useCallback((hex: string) => {
    skipSync.current = true;
  ...
  const handleSV = useCallback((e: React.PointerEvent, start = false) => {
  ...
  const handleHue = useCallback((e: React.PointerEvent, start = false) => {
  ...
  const stopDrag = useCallback(() => {
    if (isSvDrag.current || isHueDrag.current) {
      addRecentColor(_hsvToHex(hue, sat, bright));
      setRecent(getRecentColors());
    }
    isSvDrag.current = false;
  ...
  const handleRgbChange = useCallback((channel: 'r' | 'g' | 'b', val: string) => {
    ...
    setRgb(newRgb);
    emitHex(_rgbToHex(newRgb.r, newRgb.g, newRgb.b));
  }, [rgb, emitHex]);

  const handleHexSubmit = useCallback(() => {
    const h = hexInput.startsWith('#') ? hexInput : `#${hexInput}`;
    if (/^#[0-9a-fA-F]{6}$/.test(h)) emitHex(h);
    else setHexInput(value);
  }, [hexInput, value, emitHex]);

  const PRESETS = ['#22c55e', '#ef4444', '#3b82f6', '#fbbf24', '#a855f7', '#06b6d4', '#ec4899', '#f97316', '#ffffff', '#525252'];
  const inputCls = 'w-full h-5 text-[9px] font-mono text-center rounded px-0.5 bg-[rgba(255,255,255,0.04)] text-[rgba(255,255,255,0.7)] border border-[rgba(255,255,255,0.08)] focus:border-[rgba(255,255,255,0.2)] focus:outline-none';

  return (
    <div>
      {/* SV square */}
      <div className="relative mb-1.5 rounded overflow-hidden" style={{ height: 110, border: '1px solid rgba(255,255,255,0.06)' }}>
        <canvas ref={svRef} width={200} height={110} className="w-full h-full cursor-crosshair"
        ...
        <div className="absolute pointer-events-none" style={{
          ...
          borderRadius: '50%', border: '1.5px solid #fff', boxShadow: '0 0 3px rgba(0,0,0,0.9), inset 0 0 1px rgba(0,0,0,0.3)',
        }} />
      </div>

      {/* Hue bar */}
      <div className="relative mb-2 rounded overflow-hidden" style={{ height: 14, border: '1px solid rgba(255,255,255,0.06)' }}>
        <canvas ref={hueRef} width={200} height={14} className="w-full h-full cursor-ew-resize"
        ...
        <div className="absolute top-0 pointer-events-none" style={{
          left: `${(hue / 360) * 100}%`, width: 4, height: '100%',
      ...
      <div className="flex items-center gap-1.5 mb-2">
        <div className="w-6 h-6 rounded flex-shrink-0" style={{
          backgroundColor: value, boxShadow: 'inset 0 0 0 1px rgba(255,255,255,0.1)',
        }} />
        <div className="flex-1">
          <input type="text" value={hexInput.toUpperCase()} spellCheck={false}
            onChange={(e) => { let v = e.target.value; if (!v.startsWith('#')) v = '#' + v; if (v.length <= 7) setHexInput(v); }}
            onBlur={handleHexSubmit}
            onKeyDown={(e) => { if (e.key === 'Enter') handleHexSubmit(); }}
            className={inputCls} />
        </div>
        <button type="button" onClick={handleCopy} title="Copy HEX"
          className="w-6 h-5 flex items-center justify-center rounded text-[9px] hover:bg-[rgba(255,255,255,0.06)] transition-colors"
      ...
      <div className="grid grid-cols-3 gap-1 mb-2">
        {(['r', 'g', 'b'] as const).map(ch => (
          <div key={ch}>
            <label className="block text-[8px] text-center uppercase mb-0.5" style={{ color: 'rgba(255,255,255,0.3)' }}>
              {ch}
            </label>
      ...
      <div className="flex gap-1 flex-wrap mb-1.5">
        {PRESETS.map(c => (
          <button key={c} onClick={() => emitHex(c)}
            className={`w-4 h-4 rounded-sm transition-all hover:scale-110 ${value.toLowerCase() === c.toLowerCase() ? 'ring-1 ring-[rgba(255,255,255,0.5)]' : ''}`}
      ...
      {recent.length > 0 && (
        <div>
          <div className="text-[8px] uppercase mb-1" style={{ color: 'rgba(255,255,255,0.25)' }}>Recent</div>
          <div className="flex gap-1 flex-wrap">
            {recent.map((c, i) => (
              <button key={`${c}-${i}`} onClick={() => emitHex(c)}
                className="w-4 h-4 rounded-sm transition-all hover:scale-110"
                style={{ backgroundColor: c, boxShadow: 'inset 0 0 0 1px rgba(255,255,255,0.06)' }} />
            ))}
          </div>
        </div>
      )}
