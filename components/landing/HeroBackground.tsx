...
const ORBITS = [
  { rot: 0, color: '#4ade80', op: 0.55, dur: 11 },
  { rot: 60, color: '#2dd4bf', op: 0.45, dur: 15 },
  { rot: 120, color: '#4ade80', op: 0.45, dur: 9 },
];
  ...
  return (
    <div className="brand-anim" aria-hidden="true" style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0, overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        filter: 'blur(36px)', opacity: 0.85, animation: 'brand-aurora 24s ease-in-out infinite alternate',
      }} />
        ...
        <svg viewBox="0 0 200 200" width="100%" height="100%"
          ...
          {ORBITS.map((o, i) => (
            <g key={i} transform={`rotate(${o.rot} 100 100)`}>
              <ellipse cx="100" cy="100" rx="92" ry="33" fill="none" stroke={o.color} strokeOpacity={o.op} strokeWidth="1" />
              <circle r={i === 0 ? 2.6 : 2.4} cx={anim ? 0 : 8} cy={anim ? 0 : 100} fill={o.color}>
                {anim && <animateMotion dur={`${o.dur}s`} repeatCount="indefinite" path={ATOM_PATH} />}
              </circle>
            </g>
          ))}
          <circle cx="100" cy="100" r="4" fill="#4ade80" opacity="0.7" />
        </svg>
      </div>
      ...
      {PARTICLES.map((p, i) => (
        <i key={i} style={{
          ...
          borderRadius: '50%', background: '#4ade80', boxShadow: '0 0 4px rgba(74,222,128,.6)', opacity: 0.2,
