...
export default function FootprintTESTChart({ symbol = 'BTCUSDT', tickSize = 10 }: Props) {
  ...
  function renderAll(ctx: CanvasRenderingContext2D, W: number, H: number) {
    ...
    // ── Volume profile overlay (inside chart clip, right-anchored) ───────────
    renderVolumeProfileOverlay(ctx, chartW, chartH, chartY, sessionBid, sessionAsk, sessionPOC, priceMin, pRange, tickSize, toY);
  ...
  function renderVolumeProfileOverlay(
    ...
    // VAH / VAL full-width dashed lines
    ...
    ctx.beginPath();
    ctx.moveTo(0, toY(poc)); ctx.lineTo(chartW, toY(poc));
    ctx.stroke();
    ctx.globalAlpha = 1;
