//! M2 smoke test — connects to Binance Futures USDT-M public WS and
//! prints every aggTrade for 30 seconds, then disconnects.
//!
//! Usage (PowerShell):
//!   cargo run --example binance_subscribe
//!
//! Optional overrides via env:
//!   $env:BINANCE_SYMBOL = "ETHUSDT"

use std::time::{Duration, Instant};

use desktop_lib::connectors::adapter::MarketDataAdapter;
use desktop_lib::connectors::binance::BinanceAdapter;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    adapter.subscribe(&symbol, "binance").await?;

    println!(
        "\n=== Streaming Binance {} for {}s ===\n",
        symbol,
    ...
    adapter.unsubscribe(&symbol, "binance").await?;
