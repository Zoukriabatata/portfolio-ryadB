//! Low-level WebSocket transport for Binance.
//!
//! Mirrors the state machine from `connectors::rithmic::client`
//! (Disconnected → Whole → Split) but speaks JSON text frames instead
//! of protobuf binary. Subscriptions are dynamic: SUBSCRIBE /
//! UNSUBSCRIBE messages can be sent any time on the same socket.
