...
// ladder where each row sits at the SAME y-position as its price in
// the adjacent chart.
//
// Alignment math
//
//   priceToY(price) = areaTopPx + ((maxPrice - price) / (maxPrice -
//                     minPrice)) * areaHeightPx
//
// The `priceMap` prop is polled from FootprintCanvas via the
// `getPriceMap()` imperative method on RAF, so pan / zoom / resize
// keep the ladder synced with the chart without explicit re-mounts.
// When `priceMap` is null (chart not yet laid out) we fall back to a
// "stacked" ladder so the user still sees something.
...
type Props = {
  symbol: string;
  /** Optional: number of levels to display on each side. Default 15. */
  depth?: number;
  /** When provided, rows are positioned absolutely so they line up
   *  with their matching price on the chart's price axis. */
  priceMap: PriceMap | null;
};

export function BridgeDomPanel({ symbol, depth = 15, priceMap }: Props) {
  const [snap, setSnap] = useState<DepthSnapshot | null>(null);
  const lastSeenRef = useRef<number>(0);
  const [tickCounter, setTickCounter] = useState(0);

  // Seed snapshot on mount + every time the symbol changes.
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      try {
...
/** Rows positioned absolutely at their price-y so they line up with
 *  the chart's price axis. Out-of-band levels (above maxPrice / below
 *  minPrice in the chart's current view) are skipped. */
function AlignedLadder({
  ...
  // Same Y mapping as the chart's renderer. We pass `areaTopPx` raw
  // because the canvas DOM and the DOM-panel DOM share the same
  // parent's top edge (sibling flex children) — both measure from
  // the same baseline.
...
/** Fallback when the chart hasn't reported a price map yet (cold
 *  paint, brand-new symbol). Stacks levels into two halves without
 *  any chart alignment. */
function StackedLadder({
  ...
  return (
    ...
    >
      ...
      />
      <div
        style={{ height: 1, background: PALETTE.divider, flexShrink: 0 }}
      />
      <StackedSide
        levels={bids}
        maxVol={maxVol}
