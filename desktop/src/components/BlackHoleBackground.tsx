// Black hole background canvas — port of the website's
// `components/canvas/BlackHole.tsx` adapted for the Tauri desktop:
//   • palette hardcoded to Senzoukria green (no theme store dep)
//   • no scroll parallax (the desktop welcome doesn't scroll deeply)
//   • viewport-sized canvas (re-rendered on resize)
//
// The visual recipe is identical to the website: layered nebula clouds,
// twinkling stars with parallax depth, dust motes, secondary black
// holes, horizontal accretion disk with Doppler shift, vertical lensing
// ring, infalling streaks, photon sphere, relativistic jets,
// gravitational pulse rings, shooting stars.

import { useEffect, useRef } from "react";
import { isAppActive, onAppActiveChange } from "../lib/usePauseOnBlur";

// Senzoukria brand palette — hardcoded so this component is theme-free.
const PRIMARY_RGB: [number, number, number] = [126, 211, 33];   // #7ed321
const PRIMARY_LIGHT_RGB: [number, number, number] = [163, 230, 53]; // #a3e635
const PRIMARY_DARK_RGB: [number, number, number] = [95, 163, 26];    // #5fa31a
const ACCENT_RGB: [number, number, number] = [45, 212, 191];   // #2dd4bf
const ACCENT_LIGHT_RGB: [number, number, number] = [94, 234, 212];   // #5eead4
...
export function BlackHoleBackground() {
  ...
  useEffect(() => {
    ...
    const pr = (a: number) => `rgba(${PRIMARY_RGB[0]},${PRIMARY_RGB[1]},${PRIMARY_RGB[2]},${a})`;
    const pl = (a: number) => `rgba(${PRIMARY_LIGHT_RGB[0]},${PRIMARY_LIGHT_RGB[1]},${PRIMARY_LIGHT_RGB[2]},${a})`;
    const pd = (a: number) => `rgba(${PRIMARY_DARK_RGB[0]},${PRIMARY_DARK_RGB[1]},${PRIMARY_DARK_RGB[2]},${a})`;
    const ac = (a: number) => `rgba(${ACCENT_RGB[0]},${ACCENT_RGB[1]},${ACCENT_RGB[2]},${a})`;
    const al = (a: number) => `rgba(${ACCENT_LIGHT_RGB[0]},${ACCENT_LIGHT_RGB[1]},${ACCENT_LIGHT_RGB[2]},${a})`;
    const pHue = rgbToHue(...PRIMARY_RGB);

    const motionQuery = window.matchMedia("(prefers-reduced-motion: reduce)");
    let prefersReducedMotion = motionQuery.matches;
    const isMobile = window.innerWidth < 768;

    let w = 0, viewH = 0, cx = 0, cy = 0, R = 0, dpr = 1;
    ...
    // Particle counts halved vs the website to keep desktop CPU/GPU
    // budget low when the user runs a foreground game in parallel.
    // The visual recipe is preserved (same gradients, same animations);
    // we just sample fewer points per pass.
    // ── Stars (parallax depth via twinkle phase) ──────────────────────────
    const bgStars = Array.from({ length: isMobile ? 200 : 350 }, () => {
    ...
    const secondaryBHs = [
      { normX: 0.08, yMul: 0.70, radiusFrac: 0.18, intensity: 0.85, phase: 0,   pulse: 0.0007, absX: 0, absY: 0, absR: 0 },
      { normX: 0.89, yMul: 0.35, radiusFrac: 0.05, intensity: 0.35, phase: 3.1, pulse: 0.0016, absX: 0, absY: 0, absR: 0 },
      { normX: 0.91, yMul: 0.85, radiusFrac: 0.10, intensity: 0.60, phase: 1.8, pulse: 0.0011, absX: 0, absY: 0, absR: 0 },
    ];
    ...
    // ── Horizontal accretion disk (rotating, with Doppler shift) ──────────
    const diskCount = isMobile ? 150 : 300;
    ...
    // ── Vertical lensing ring (gravitational lensing illusion) ────────────
    const lensRingCount = isMobile ? 60 : 150;
    ...
    const streaks = Array.from({ length: isMobile ? 6 : 12 }, () => ({
      angle: Math.random() * Math.PI * 2,
    ...
    // ── Shooting stars ────────────────────────────────────────────────────
    const shootingStars = Array.from({ length: isMobile ? 3 : 6 }, () => ({
    ...
    const spawnShootingStar = (s: typeof shootingStars[0]) => {
      ...
      if (side < 0.5) { s.x = Math.random() * w; s.y = -10; }
      else { s.x = side < 0.75 ? -10 : w + 10; s.y = Math.random() * viewH * 0.5; }
    ...
    const gravPulses = Array.from({ length: 3 }, () => ({
    ...
    const projDisk = (angle: number, orbitR: number, yOff: number) => {
      ...
      return { px: cx + x3 * R, py: cy + z3 * TILT * R + yOff * R, depth: z3, cosA: Math.cos(angle) };
    };
    const projLens = (angle: number, orbitR: number) => {
      const x3 = Math.cos(angle) * orbitR * 0.35;
      const y3 = Math.sin(angle) * orbitR;
      return { px: cx + x3 * R, py: cy - y3 * R, depth: Math.cos(angle) };
    };
    ...
    const draw = () => {
      ...
      dustMotes.forEach((d) => {
        ...
        ctx.beginPath();
        ctx.arc(d.x % w, d.y * viewH, d.r, 0, Math.PI * 2);
        ctx.fill();
      });
      ...
      gravPulses.forEach((p) => {
        if (!p.active) return;
        p.radius += p.speed;
        const progress = (p.radius - R) / (p.maxRadius - R);
        const a = p.alpha * (1 - progress) * (1 - progress);
        if (a < 0.001 || p.radius > p.maxRadius) {
          p.active = false;
          return;
        }
        ctx.save();
        ...
        ctx.shadowBlur = 10;
        ctx.beginPath();
        ctx.ellipse(cx, cy, p.radius, p.radius * TILT, 0, 0, Math.PI * 2);
        ctx.stroke();
        ctx.restore();
      });
      ...
      // Skip RAF scheduling when the OrderflowV2 window isn't focused
      // (alt-tab to Valorant etc.) — saves ~95% of GPU/CPU instantly.
      if (!prefersReducedMotion && isAppActive()) {
    ...
    const onMotionChange = (e: MediaQueryListEvent) => {
      ...
      else if (isAppActive()) raf = requestAnimationFrame(draw);
    };
    motionQuery.addEventListener("change", onMotionChange);

    // Resume RAF the moment the user alt-tabs back into the app.
    const offActive = onAppActiveChange((nowActive) => {
      if (!nowActive) {
  ...
  return (
    <canvas
      ref={canvasRef}
      className="senz-blackhole-canvas"
      aria-hidden
    />
  );
}
