...
import type { LayoutMetrics } from '../orderflow/FootprintLayoutEngine';
...
export class FootprintCanvasRenderer {
  ...
  renderFootprintCandles(params: RenderParams): void {
    ...
    // Dynamic font scaling with zoom (Phase 2). Upper cap raised so
    // bid×ask labels and per-candle delta text scale up when the user
    // zooms in (or sits back from the monitor) instead of plateauing
    // at a tiny 14px regardless of available cell width.
    const baseFontSize = fonts.volumeFontSize || 10;
    const fontSize = Math.max(7, Math.min(20, Math.round(baseFontSize * zoom * 0.9)));
    const fontFamily = fonts.volumeFont || '"Consolas", "Monaco", "Courier New", monospace';
    const boldPrefix = fonts.volumeFontBold ? 'bold ' : '';
    const _monoFont = `${boldPrefix}${fontSize}px ${fontFamily}`;
    const _boldMonoFont = `bold ${fontSize}px ${fontFamily}`;
    void _monoFont; void _boldMonoFont;
    ...
    metrics.visibleCandles.forEach((candle, idx) => {
      ...
      // === Empty-levels candle → full-width OHLC fallback ============
      // Bars from TimeBarReplay (history) carry OHLCV but no per-price
      // bid/ask split. Instead of leaving the slot empty (the old
      // "0 x 0 grid" symptom), we draw a regular OHLC candle that
      // fills the entire footprint width so the user sees the price
      // action at the same visual scale as live bars. Opacity is
      // reduced to 0.6 to communicate "historical context, not live
      // orderflow".
      if (candle.levels.size === 0) {
        const slotWidth = ohlcWidth + fpWidth;
        ctx.save();
        ctx.globalAlpha = 0.6;
        this.renderOHLCCandle(
          ctx, layout, metrics, colors, candle, fpX, slotWidth,
        );
        ctx.restore();
        return;
      }
      ...
      candle.levels.forEach((level, price) => {
        ...
        // Adaptive font — bounded by the cell's vertical space, with a
        // generous upper cap so values stay readable from a distance
        // when the user zooms out. Width-side overflow is then caught
        // by the `requiredW > availableW` shrinking step below, which
        // proportionally rescales to fit (and skips text entirely if
        // the scaled size dips below 6px).
        const cellFontSize = Math.max(7, Math.min(22, Math.floor(cellSpan * 0.72)));
  ...
  renderVWAPTWAP(
    ...
    const _twapColor = features.twapColor || '#5eaeff';
    void _twapColor;
  ...
  renderVolumeProfile(
    ...
    const _outsideColor = features?.volumeProfileOutsideColor || '#3a3f4b';
    void _outsideColor;
    ...
    const _barGap = Math.max(0.5, (rowH - barH) * 0.3);
    void _barGap;
  ...
  renderVolumeProfileOverlay(
    ...
    valColor = '#7ed321',
    vaBarColor = '#7ed321',
    outsideBarColor = 'rgba(255, 255, 255, 0.65)',
  ): void {
    ...
    // ── VAH (white) / VAL (green) full-width solid lines ──────────────────────
    ...
    totalVol.forEach((vol, price) => {
      ...
      ctx.globalAlpha = isPOC ? 0.85 : isVA ? 0.55 : 0.30;
      ctx.fillStyle   = isPOC ? pocColor : isVA ? vaBarColor : outsideBarColor;
      ctx.fillRect(footprintAreaX, y1, barW, barH);
    });

    ctx.restore();

    // ── VAH (white) / VAL (green) labels ──────────────────────────────────────
    ctx.globalAlpha  = 0.85;
    ctx.font         = 'bold 7px "Consolas", monospace';
    ctx.textBaseline = 'alphabetic';
    ctx.textAlign    = 'left';
    if (vahY > footprintAreaY + 10) {
      ctx.fillStyle = vahColor;
  ...
  renderPriceScale(
    ...
    // Background — widened to 80px so BTC-class labels ($80,886.50) fit
    // without overflowing the canvas. Uses the same width as layoutCfg.priceAxisWidth
    // in the desktop adapter.
    const SCALE_W = 80;
    ctx.fillStyle = colors.surface;
    ctx.fillRect(width - SCALE_W, footprintAreaY, SCALE_W, footprintAreaHeight);

    // Left border
    ctx.strokeStyle = colors.gridColor;
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(width - SCALE_W, footprintAreaY);
    ctx.lineTo(width - SCALE_W, footprintAreaY + footprintAreaHeight);
    ctx.stroke();
    ...
    for (const price of labels) {
      ...
      ctx.strokeStyle = colors.gridColor;
      ctx.beginPath();
      ctx.moveTo(width - SCALE_W, y);
      ctx.lineTo(width - SCALE_W + 4, y);
      ctx.stroke();
  ...
  renderSessionHeader(
    ...
    // Semi-transparent background — width-80 to not overflow the price scale.
    ctx.fillStyle = 'rgba(10, 10, 10, 0.75)';
    ctx.fillRect(0, headerY, width - 80, headerH);
    ...
    ctx.lineTo(width - 80, headerY + headerH);
    ctx.stroke();

    // Stats text — Senzoukria palette: white labels, green values, white for neutrals.
    ...
    // Delta — green if positive, white if negative (no red in the palette).
    ctx.fillStyle = LABEL;
    ctx.fillText('Delta:', x, textY);
    x += 32;
    ctx.fillStyle = sessionStats.totalDelta >= 0 ? ACCENT : VALUE;
  ...
  renderFooterStatusBar(
    ...
    ctx.textAlign = 'right';
    const fpsColor = this._currentFps >= 50 ? '#7ed321' : this._currentFps >= 30 ? 'rgba(255,255,255,0.65)' : '#ffffff';
  ...
  renderAlertLines(
    ...
    for (const alert of alerts) {
      ...
      const color = alert.direction === 'above' ? '#7ed321' : '#ffffff';
  ...
  renderStackedImbalances(
    ...
    layout: FootprintLayoutEngine,
    metrics: LayoutMetrics,
    imbalances: import('./rendererTypes').StackedImbalance[],
    rowH: number,
    fpWidth: number,
    ohlcWidth: number,
    showOHLC: boolean,
    ...
    for (const si of imbalances) {
      ...
      ctx.fillStyle = isBullish ? '#7ed321' : '#ffffff';
      ctx.globalAlpha = 0.12;
      ctx.fillRect(cellStartX + 1, startY, fpWidth - 2, rectH);
      ctx.globalAlpha = 1;

      // Border
      ctx.strokeStyle = isBullish ? '#7ed321' : '#ffffff';
      ctx.lineWidth = 1;
      ctx.globalAlpha = 0.4;
  ...
  renderUnfinishedAuctions(
    ...
    layout: FootprintLayoutEngine,
    metrics: LayoutMetrics,
    auctions: import('./rendererTypes').UnfinishedAuction[],
    width: number,
  ): void {
    ...
    metrics.visibleCandles.forEach((candle, idx) => {
      ...
      rows.forEach(({ type }, i) => {
        ...
        if (type === 'time') {
          ...
          // Intensity gauge — proportional strip at the BOTTOM of the
          // cell instead of a full-row tint. Two benefits:
          //  1. Crisp delimitation: the strip has visible edges, so
          //     the user can read "how strong is this delta" at a
          //     glance instead of parsing a faint background tint.
          //  2. Zero overlap with the value text — the strip sits in
          //     the bottom 2 px, the value text baseline shifts up
          //     1 px to give it breathing room.
          const deltaIntensity = Math.min(1, Math.abs(delta) / Math.max(1, volume) * 2);
          if (delta !== 0 && deltaIntensity > 0.02) {
            ctx.fillStyle = delta > 0
              ? (colors.clusterDeltaPositive ?? colors.deltaPositive)
              : (colors.clusterDeltaNegative ?? colors.deltaNegative);
            ctx.globalAlpha = 0.85;
            const barH = 2;
            const barW = Math.max(2, Math.round((cellW - 4) * deltaIntensity));
            ctx.fillRect(
              cellX + (cellW - barW) / 2,
              rowY + rowH - barH,
              barW,
          ...
          ctx.fillText(formatVal(delta), centerX, rowY + rowH - 5);
        } else if (type === 'vol') {
