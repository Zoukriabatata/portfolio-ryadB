...
import type { FootprintCandle } from './types';
...
export class FootprintLayoutEngine {
   ...
   * Renvoyée en sub-pixel (FLOAT, pas arrondie) pour que le zoom et le
   * drag soient parfaitement continus. Les rendus individuels (candle
   * body, container, cells) snappent eux-mêmes au pixel via Math.round
   * pour rester nets — l'arrondi se fait au point de dessin, pas au
   * niveau de la métrique de layout.
   */
  ...
  scroll(deltaPixels: number): void {
    ...
    // Negative scrollX is treated as a right margin in getFootprintX
    // (see setScroll); only the positive part drives the past-scroll
    // slice via scrollIndex.
    this.viewport.scrollIndex = Math.floor(
      Math.max(0, this.viewport.scrollX) / effectiveWidth,
    );
  }
   ...
   *
   * Negative `scrollX` is treated as a *right margin* — it pulls the
   * entire chart left so the newest candle slides toward the centre
   * of the viewport, but the underlying slice of bars stays the same
   * (no phantom array indices, no slice churn). Positive `scrollX`
   * scrolls into the past as before.
   */
  setScroll(scrollX: number, scrollY: number): void {
    this.viewport.scrollX = scrollX;
    this.viewport.scrollY = scrollY;
    // Convert positive scrollX to scrollIndex for past-scroll. Negative
    // values keep scrollIndex at 0 — handled via rightMargin in
    // `getFootprintX` instead.
    const fpWidth = this.getEffectiveFootprintWidth();
    this.viewport.scrollIndex = Math.floor(Math.max(0, scrollX) / fpWidth);
  }
   ...
   */
  resetToOptimalView(_totalCandles: number): void {
    this.viewport.scrollX = 0;
  ...
  getFootprintX(visibleIndex: number, metrics: LayoutMetrics): number {
    ...
    // Smooth-drag fractional offset.
    //
    // scrollX advances in PIXELS (driven by the mouse-drag handler),
    // but `scrollIndex = floor(scrollX / effectiveWidth)` only ticks
    // forward when a full slot is crossed. Without compensating, the
    // chart appears frozen for ~15px of drag and then suddenly jumps
    // one whole bar — the "bar-by-bar choppy drag" symptom.
    //
    // The fix: apply the sub-slot remainder to every bar's X so the
    // whole grid slides pixel-perfectly with the cursor while the
    // integer index increments only on slot boundaries. Only the
    // POSITIVE part of scrollX drives this — negative values are
    // handled separately as a right margin.
    const positiveScroll = Math.max(0, this.viewport.scrollX);
    const remainder =
      effectiveWidth > 0
        ? ((positiveScroll % effectiveWidth) + effectiveWidth) % effectiveWidth
        : 0;

    // Right margin — when the user has dragged past the most recent
    // bar, `scrollX` goes negative and we shift every bar LEFT by
    // |scrollX| so the newest candle slides toward the centre of
    // the viewport, leaving empty space on the right.
    const rightMargin = Math.max(0, -this.viewport.scrollX);
    ...
    if (candleCount >= viewportCapacity) {
      // Right-aligned: last candle touches right edge (minus margin).
      const totalWidth = candleCount * effectiveWidth;
      const startX =
        metrics.footprintAreaX + metrics.footprintAreaWidth - totalWidth - rightMargin;
      return startX + visibleIndex * effectiveWidth + remainder;
    } else {
      // Left-aligned: first candle starts at left edge (no empty space).
      // Right margin still applies so the chart can be pulled away
      // from the price axis even when there aren't enough bars to
      // fill the viewport.
      return metrics.footprintAreaX + visibleIndex * effectiveWidth + remainder - rightMargin;
    }
  }
   ...
   */
  getDeltaProfilePosition(_metrics: LayoutMetrics): { x: number; width: number } {
   ...
   */
  getVolumeProfilePosition(_metrics: LayoutMetrics): { x: number; width: number } {
