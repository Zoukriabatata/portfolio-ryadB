...
// Source of truth = /pricing PRO_FEATURES. Kept in sync so the upsell never
// promises features the pricing page doesn't.
const PRO_FEATURES = [
...
function UpgradeContent() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-16 relative overflow-hidden" style={{ backgroundColor: 'var(--background)' }}>
      {/* Background orbs */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full pointer-events-none"
        style={{ background: 'radial-gradient(circle, rgb(var(--primary-rgb) / 0.07), transparent 65%)', filter: 'blur(80px)' }} />
      <div className="absolute bottom-0 right-0 w-[400px] h-[400px] rounded-full pointer-events-none"
        style={{ background: 'radial-gradient(circle, rgb(var(--accent-rgb) / 0.05), transparent 65%)', filter: 'blur(80px)' }} />
      ...
      <div className="relative z-10 w-full max-w-2xl mx-auto">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl mb-6"
            style={{ background: 'rgb(var(--primary-rgb) / 0.08)', border: '1px solid rgb(var(--primary-rgb) / 0.2)' }}>
          ...
          {isExpired ? (
            <>
              <h1 className="text-2xl md:text-3xl font-display text-[var(--text-primary)] mb-3">
            ...
            <>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-[11px] font-medium mb-4"
                style={{ background: 'rgb(var(--primary-rgb) / 0.08)', color: 'var(--primary-light)', border: '1px solid rgb(var(--primary-rgb) / 0.15)' }}>
              ...
              <p className="text-[var(--text-muted)] text-sm leading-relaxed max-w-sm mx-auto">
                <span style={{ color: 'var(--primary-light)' }}>{featureInfo.name}</span> is only available on the PRO plan.
        ...
        <div className="rounded-2xl p-6 mb-6 relative overflow-hidden"
          style={{ background: 'rgb(var(--primary-rgb) / 0.04)', border: '1px solid rgb(var(--primary-rgb) / 0.15)' }}>
          {/* Launch badge */}
          <div className="absolute top-4 right-4">
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-semibold tracking-wide uppercase"
              style={{ background: 'rgb(var(--primary-rgb) / 0.12)', color: 'var(--primary-light)', border: '1px solid rgb(var(--primary-rgb) / 0.25)' }}>
          ...
          <div className="flex items-end gap-2 mb-1">
            <span className="text-4xl font-black text-[var(--text-primary)]">$29</span>
            <span className="text-[var(--text-muted)] text-sm mb-1.5">/month</span>
          </div>
          <p className="text-[12px] text-[var(--text-dimmed)] mb-5">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <Link
            href="/pricing"
            className="btn-brand w-full sm:w-auto flex-1 inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl text-sm font-semibold transition-all duration-200 hover:-translate-y-0.5"
          >
          ...
          <Link
            href="/"
            className="btn-brand-ghost w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl text-sm font-medium transition-all duration-200"
          >
            Back to Home
          </Link>
        </div>
        ...
        {/* Reassurance */}
        <p className="text-center text-[11px] text-[var(--text-dimmed)] mt-5">
