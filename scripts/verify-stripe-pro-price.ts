 ...
 *
 * Usage:
 *   npx tsx scripts/verify-stripe-pro-price.ts
 *
...
(async () => {
  ...
  if (price.livemode)                                                  issues.push('LIVEMODE is true (expected test mode)');
  ...
  console.log('\n[verify] OK — price matches spec ($29.00 USD/month, test mode, active)');
})().catch(err => {
